...
x['image'] = [osp.join('data/jpeg/train/', _) for _ in x['image']]
...
# Combine everything
df = pd.concat([x,y])
df.to_csv('../../data/complete_combined_train_with_splits.csv', index=False)

# Multiply melanoma from ISIC20 by 8
df = pd.concat([x,y]+[x[x['target'] == 1]]*7)
df.to_csv('../../data/upsampled_combined_train_with_splits.csv', index=False)

# Only add melanomas from ISIC2019
df = pd.concat([x,y[y['label'] == 1]]+[x[x['target'] == 1]]*7)
df.to_csv('../../data/upsampled_combined_mel_train_with_splits.csv', index=False)

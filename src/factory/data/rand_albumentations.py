...
def TranslateX(img, v):  # [-150, 150] => percentage: [-0.45, 0.45]
    ...
    if random.random() > 0.5:
        v = -v
    return getattr(albu, 'ShiftScaleRotate')().apply(img, dx=v)
...
def ZoomIn(img, v):
    assert 0 <= v <= 0.5
    return getattr(albu, 'ShiftScaleRotate')().apply(img, scale=v)


def ZoomOut(img, v):
    assert 0 <= v <= 0.5
    return getattr(albu, 'ShiftScaleRotate')().apply(img, scale=v-1)
...
def Rotate(img, v):  # [-30, 30]
    ...
    if random.random() > 0.5:
        v = -v
    return getattr(albu, 'ShiftScaleRotate')().apply(img, angle=v)
